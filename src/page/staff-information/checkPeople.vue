<template >
  <div class="myapp-check myapp app" v-if='show'>
...
<script>
import { TransferDom, Flexbox, FlexboxItem, Swipeout, SwipeoutItem, SwipeoutButton } from 'vux';
...
export default {
  ...
  created(){
    this.routeParams = this.phids.split(',');
  ...
  methods: {
    change (index) {
      ...
      this.checkAll = (this.list.length == this.checkedNumber) ? true: false;
    },
    checkall() {
      let that = this;
    ...
    submit() {
      ...
      this.phids = phids;
      this.$emit('phids', phids);
      this.onChange && self.onChange(phids, length);
      this.$emit('onChange', phids, length);
    },
    onRefresh(done) {
      this.list = [];
      this.loadmore = true;
